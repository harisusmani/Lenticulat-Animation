...
void renderMyDesign (float percent) {
  ...
  fill (100); 
  float Big_R=350 *(width/500)/2; //Outer Circle Radius
  ellipse (cx, cy, Big_R*2, Big_R*2); //Outer Circle
  //fill (ellipseColor, ellipseColor, ellipseColor);
  fill (80);  
  float R=150 * (width/500); //Inner Circle Radius
  ...
  { 
    radius = (float)map(percent,stage1,stage2,50* (width/500),130* (width/500));
    fill (0, 0, 0);
    polygon(nSpokes, 0, 0, radius);
  }
  else if (percent<stage3) //Stage3, Adjust/Focus
  {
    radius = (float)map(percent,stage2,stage3,130* (width/500),25* (width/500));
    fill (0, 0, 0);
    polygon(nSpokes, 0, 0, radius);
  }
  else
  {
    radius = (float)map(percent,stage3,1,25* (width/500),50* (width/500)); //Stage 4, Fine Focus & Loop GIF
    fill (0, 0, 0);
    polygon(nSpokes, 0, 0, radius);
  }
